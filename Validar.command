...
[[ -f "$ROOT_DIR/RC312_FROZEN_SHA256.txt" ]]
...
printf '  Verificando congelación completa de las fuentes RC3.12...\n'
(cd "$ROOT_DIR" && shasum -a 256 -c RC312_FROZEN_SHA256.txt)
...
if grep -REq 'GameModeController|gamepolicyctl|game-mode[[:space:]]+set' "$ROOT_DIR/Sources"; then
  echo "ERROR: reapareció la integración Game Mode retirada en RC3.12."
  exit 1
fi
...
printf '\nVALIDACIÓN COMPLETADA: ThermalBridge %s RC3.12 (%s)\n' "$VERSION" "$BUILD"
