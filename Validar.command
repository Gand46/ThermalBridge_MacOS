...
[[ -f "$ROOT_DIR/RC38_FROZEN_SHA256.txt" ]]
...
printf '  Verificando congelación completa de las fuentes RC3.8...\n'
(cd "$ROOT_DIR" && shasum -a 256 -c RC38_FROZEN_SHA256.txt)
...
if grep -REq 'GameModeController|gamepolicyctl|game-mode[[:space:]]+set' "$ROOT_DIR/Sources"; then
  echo "ERROR: reapareció la integración Game Mode retirada en RC3.8."
  exit 1
fi
...
printf '\nVALIDACIÓN COMPLETADA: ThermalBridge %s RC3.8 (%s)\n' "$VERSION" "$BUILD"
