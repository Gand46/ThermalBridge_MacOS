...
[[ -f "$ROOT_DIR/RC311_FROZEN_SHA256.txt" ]]
...
printf '  Verificando congelación completa de las fuentes RC3.11...\n'
(cd "$ROOT_DIR" && shasum -a 256 -c RC311_FROZEN_SHA256.txt)
...
if grep -REq 'GameModeController|gamepolicyctl|game-mode[[:space:]]+set' "$ROOT_DIR/Sources"; then
  echo "ERROR: reapareció la integración Game Mode retirada en RC3.11."
  exit 1
fi
...
printf '\nVALIDACIÓN COMPLETADA: ThermalBridge %s RC3.11 (%s)\n' "$VERSION" "$BUILD"
